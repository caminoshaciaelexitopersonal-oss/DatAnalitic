...
class StateStore:
    def __init__(self):
        ...
        self._initialize_db()
        # Redis and MinIO connections... (same as before)
        try:
    ...
    def create_job(self, session_id: uuid.UUID, job_type: str, filename: str) -> Optional[JobModel]:
        with self.session_scope() as db:
    ...
    def create_mcp_step(self, job_id: uuid.UUID, description: str, payload: Optional[Dict]) -> Optional[MCPStepModel]:
        with self.session_scope() as db:
    ...
    # --- Scoreboard Method ---
    def update_scoreboard(self, trial_result: Dict[str, Any]):
        with self.session_scope() as db:
    ...
    def save_raw_file(self, job_id: str, filename: str, content: bytes):
        """Saves the original uploaded file to MinIO."""
    ...
    def load_raw_file(self, job_id: str, filename: str) -> Optional[BytesIO]:
        """Loads the original uploaded file from MinIO."""
        try:
            key = f"raw/{job_id}/{filename}"
