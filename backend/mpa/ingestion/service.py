...
class IngestionService:
    ...
    async def process_directory(self, directory_path: str) -> List[pd.DataFrame]:
        """Processes all supported files in a given directory, returning a list of DataFrames."""
        dataframes = []
        for filename in sorted(os.listdir(directory_path)):
            ...
            if os.path.isfile(file_path):
                ...
                df = await self._read_and_process_file_content(file_path, content)
                if df is not None:
    ...
    async def process_uploaded_file(self, file: UploadFile, job_id: str) -> pd.DataFrame:
        ...
        # Assuming a similar save method exists for quality reports by job_id
        # self.state_store.save_quality_report(job_id, quality_report.model_dump())
